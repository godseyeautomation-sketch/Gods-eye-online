...
export const AutopilotPage: React.FC<AutopilotPageProps> = ({ onNavigate }) => {
  ...
  // Pre-Review destination chooser: Slack vs Dashboard
  ...
  // ── Re-fetch Slack connection status when Review-mode modal opens ─────
  useEffect(() => {
  ...
  return (
    <div className="h-full flex overflow-hidden">
                    ...
                    <div className="px-4 pb-3 space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            // For Review specifically, pop the destination chooser
                            // (Slack vs Dashboard). Other agents run directly.
      ...
      {/* ── Pre-Review destination chooser ─────────────────────────────── */}
      {showReviewModeModal && (
        <div
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-text-secondary mb-6">Each post needs explicit approval — no auto-cascade.</p>
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={async () => {
                  if (!slackConnected) {
                    // Kick off OAuth flow inline — opens Slack auth in same tab
                    if (!user?.id || !selectedBrandId) return;
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-white/60 mt-0.5">
                    {slackConnected === false
                      ? 'Click to connect your workspace (2 clicks via Slack OAuth)'
                      : 'Approve / Reject / Schedule each post directly from Slack with native modals'}
                  </p>
                </div>
              </button>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-secondary mt-0.5">Review inside Gods Eye — schedule + approve from the Approval Queue</p>
                </div>
              </button>
            </div>

      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-white tracking-tight">Apps</h1>
          </div>
          <p className="text-sm text-white/40 ml-[52px]">Specialized AI tools built for creators. One-click workflows for real results.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-8">
          <input
            ...
            className="w-full sm:w-64 bg-white/[0.03] border border-white/8 rounded-xl px-4 py-2.5 text-sm text-white/80 placeholder-white/20 outline-none focus:border-white/20 transition-colors"
          />
          ...
          {filteredApps.map(app => (
            <button
              ...
              className={`group relative text-left p-5 rounded-2xl border transition-all duration-300 ${
                app.status === 'live'
                  ? 'bg-white/[0.02] border-white/[0.06] hover:bg-white/[0.05] hover:border-white/[0.12] cursor-pointer'
                  : 'bg-white/[0.01] border-white/[0.04] opacity-60 cursor-default'
              }`}
            >
              {/* Icon */}
              ...
              <div className="absolute top-4 right-4">
                {app.status === 'live' ? (
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-white/5 text-white/30 text-[9px] font-black uppercase tracking-wider border border-white/10">
                    Soon
                  </span>
                )}
              </div>
              ...
              <p className="text-[11px] text-white/35 leading-relaxed line-clamp-2 mb-3">{app.description}</p>

              {/* Footer */}
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-white/20 uppercase tracking-wider font-bold">{app.category}</span>
                {app.status === 'live' && (
